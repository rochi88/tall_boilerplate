...
<nav aria-label="Options"
    ...
    <div class="relative flex items-center flex-shrink-0 p-2" x-data="{ isOpen: false }">
        <button @click="isOpen = !isOpen; $nextTick(() => {isOpen ? $refs.userMenu.focus() : null})"
            ...
            <img class="w-8 h-8 rounded-lg shadow-md" src="{{ auth()->user()->getFirstMediaUrl('avatars') }}"
                alt="{{ auth()->user()->name }}" />
...
<nav aria-label="Options"
    ...
    <div class="flex-shrink-0 py-4">
        <a href="#">
            <img class="w-10 h-auto" src="{{ asset('apple-touch-icon.png') }}" alt="{{ config('app.name') }}" />
        </a>
    </div>
    ...
    <nav x-show="currentSidebarTab == 'linksTab'" aria-label="Main" class="flex flex-col h-full">
        ...
        <div class="flex items-center justify-center flex-shrink-0 py-10">
            <a href="#">
                ...
                <img class="w-24 h-auto" src="{{ asset('apple-touch-icon.png') }}"
                    alt="{{ config('app.name') }}" />
            </a>
        </div>
